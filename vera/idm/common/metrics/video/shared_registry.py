...
except ImportError:  # torchmetrics >= 1.9 renamed this to a private symbol
...
from .models import AMT_S, CLIP, DINO, I3D, LAION, MUSIQ, RAFT, MotionExtractor
    ...
    def _register(self, model_type: VideoMetricModelType):
        match model_type:
            ...
            case VideoMetricModelType.MUSIQ:
                if MUSIQ is None:
                    raise RuntimeError(
                        "MUSIQ (pyiqa) is not installed; it requires Python <3.10. "
                        "Install with: pip install pyiqa==0.1.10 (in a Python 3.10 env)."
                    )
